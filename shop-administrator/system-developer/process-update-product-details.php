<?php
	...
	try{
		...
		$productDetails = new ragzNationProducts($db);
	   	...
	   	$dir1 = "../../images/product/large-image/";
		...
		if(isset($_POST['update-product'])){
			...
			$product_name = $all_purpose->sanitizeInput($_POST['product_name']);
			...
			if(empty($file_name)){
				...
				if(!empty($productDetails->updateNewProductDetailsWithoutImage($product_name, $product_number, $type_id, $manufacturer_id)) AND($productDetails->updateTheProductDetailsWithOutThumbnail($product_number, $product_description, $product_price, $type_id, $quantity, $product_size, $category_id))){
				...
				if(($file_size > 2097152) OR ($file_size2 > 2097152)){
					...
					if(!empty($productDetails->updateNewProductDetailsWithImage($product_name, $product_number, $product_image, $type_id, $manufacturer_id))AND($productDetails->updateTheProductDetailsWithOutThumbnail($product_number, $product_description, $product_price, $type_id, $quantity, $product_size, $category_id))){
